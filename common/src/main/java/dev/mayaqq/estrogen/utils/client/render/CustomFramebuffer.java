...
public class CustomFramebuffer {
    ...
    public void drawToScreen(int viewWidth, int viewHeight, boolean depthAware) {
        RenderSystem.assertOnRenderThread();
        ...
        ShaderInstance shader = depthAware ? EstrogenShaders.getDepthAwareBlitShader() : Minecraft.getInstance().gameRenderer.blitShader;

        shader.setSampler("DiffuseSampler", colorAttachment);
        if(depthAware) shader.setSampler("DiffuseDepthSampler", Minecraft.getInstance().getMainRenderTarget().getDepthTextureId());
